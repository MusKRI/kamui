...
export const PureUIBodyInfo = ({ content }: Props) => {
          ...
          <div className="mt-10">
            <ul className="flex flex-col gap-3 md:gap-4">
              {features.map((feature: string, index: number) => (
                ...
                >
                  <span className="mr-2 p-1 shrink-0 bg-(--surface-3) border border-(--link)/5 text-(--muted-foreground) rounded-full relative z-2 overflow-hidden">
                    <span className="absolute size-6 rounded-full bg-gradient-to-t z-1 -top-1/4 left-1/2 -translate-x-1/2 from-(--background) to-(--link) opacity-40 blur-[2px]"></span>
                    <svg
                      viewBox="0 0 24 24"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                      className="w-4 h-4 relative z-2 text-(--foreground)"
                    >
                      <path
                        d="M20 6L9 17L4 12"
                        stroke="currentColor"
                  ...
                  <p className="relative z-2 text-base tracking-3 font-normal text-pretty m-0 my-0.5 text-(--accent-soft-foreground)">
                    {feature}
                  </p>
                </li>
              ))}
